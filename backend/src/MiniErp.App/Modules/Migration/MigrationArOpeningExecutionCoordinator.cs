...
internal sealed class MigrationArOpeningExecutionCoordinator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var row in rows.OrderBy(item => item.Staged.SourceSequence))
        {
            ...
            var unknown = responseLost || response.Code is "finance_unavailable" or "finance_ar_opening_outcome_unknown" or "migration_ar_opening_outcome_unknown";
            return await FailEffectAsync(tenant, batch, effect, response.Code, unknown, unknown ? CancellationToken.None : cancellationToken);
        }
