...
internal sealed class MigrationGlOpeningExecutionCoordinator
{
    ...
    {
        var allLines = evidence.ResidualLines.Concat(evidence.RepresentedControlLines ?? Array.Empty<FinanceApp.FinanceMigrationGlOpeningResidualLine>()).OrderBy(item => item.AccountId).ToArray();
