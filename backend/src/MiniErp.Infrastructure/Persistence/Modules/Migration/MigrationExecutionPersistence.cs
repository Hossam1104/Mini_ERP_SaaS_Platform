...
internal sealed partial class MigrationPersistence
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parent = await db.ExecutionEffects.SingleOrDefaultAsync(item =>
            ...
            && (item.RecordType == MigrationCanonicalRecordType.InventoryOpening || item.RecordType == MigrationCanonicalRecordType.ArOpening || item.RecordType == MigrationCanonicalRecordType.ApOpening), cancellationToken);
        if (parent is null)
