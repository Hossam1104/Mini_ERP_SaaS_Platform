...
async function setupReportingRoutes(page: Page): Promise<void> {
  ...
  await page.route('**/api/v1/reporting/catalogue', (route) => route.fulfill({ json: [{
    ...
    sourceOwnership: 'Finance owns posted truth.', reconciliationPath: 'Finance reconciliation', allowedFilters: ['company'], exportEnabled: true, schedulingEnabled: false, requiresCompany: true, pendingDecision: false, implementationState: 'IMPLEMENTABLE_NOW',
  }] }));
...
test.describe('MESP-139 Reporting workspace', () => {
  ...
  test('renders the English catalogue, executes a source-linked report, and keeps scope server-selected', async ({ page }) => {
    ...
    await expect(reporting.getByRole('button', { name: 'Add disabled schedule' })).toBeDisabled();
  });
